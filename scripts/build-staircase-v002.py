...
scene_name = 'Stackhouse Stair v002'
...
for k,s in enumerate(runs):
    ...
    # Two balusters per tread, seated on its actual horizontal oak surface.
    for i in range(count):
        for offset in [.25,.75]:
            t=i+offset; z=za+run*t; seat=ya+rise*(i+1)
            rail_top=ya+rise*t+.95
            beam(f'Flight {k} steel baluster {i}-{offset}',(rail_x,seat,z),(rail_x,rail_top,z),.018,.018,iron,.002)
...
# Join the two handrails at the half landing, inside the existing centre strip.
first,last=runs[0],runs[-1]
left=first['bounds']['maxX']-layout['X0']+.045
right=last['bounds']['minX']-layout['X0']-.045
turn_y=first['topY']+.95;turn_z=first['topCoord']
beam('Continuous half landing handrail return',(left,turn_y,turn_z),(right,turn_y,turn_z),.055,.065,oak,.009)
...
export_path=ROOT/'public/models/staircase-v002.glb'
