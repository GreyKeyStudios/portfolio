"""Author the Guest Bedroom with replaceable Grey Key merch surfaces. Run through Blender MCP."""
...
name='Stackhouse Guest Bedroom Furniture v002'
...
bpy.context.view_layer.objects.active=copies[0];bpy.ops.object.join();ob=bpy.context.object;triangles=sum(len(p.vertices)-2 for p in ob.data.polygons);path=ROOT/'public/models/guest-bedroom-furniture-v002.glb'
...
bpy.ops.wm.save_as_mainfile(filepath=str(SRC/'guest-bedroom-furniture-v002.blend'))
manifest={'triangles':triangles,'bytes':path.stat().st_size,'pieces':len(parts),'layout_sha256':hashlib.sha256((ROOT/'lib/guest-bedroom-furniture-v002.json').read_bytes()).hexdigest(),'merch_sha256':hashlib.sha256((ROOT/'lib/bedroom-merch-surfaces-v002.json').read_bytes()).hexdigest(),'note':'Master bedroom first, with replaceable merch image surfaces on bedding, framed art and folded apparel.'}
(SRC/'guest-bedroom-furniture-v002.manifest.json').write_text(json.dumps(manifest,indent=2));result=manifest
