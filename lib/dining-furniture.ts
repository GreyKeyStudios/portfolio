import layout from './dining-furniture-v002.json'
import { X0 } from './interior-layout'
import type { AABB } from './collision'

export const DINING_FURNITURE_COLLIDERS: AABB[] = layout.map(piece => {
  ...
  return { label: `dining-${piece.id}`, minX: X0 + piece.x - halfX,
    maxX: X0 + piece.x + halfX, minZ: piece.z - halfZ, maxZ: piece.z + halfZ }
})

let mounted = 0
const combined = new WeakMap<AABB[], AABB[]>()
export function registerDiningFurniture() {
  mounted += 1
  return () => { mounted -= 1 }
}
export function withDiningFurniture(walls: AABB[]) {
  if (!mounted) return walls
  let result = combined.get(walls)
  if (!result) { result = [...walls, ...DINING_FURNITURE_COLLIDERS]; combined.set(walls, result) }
  return result
}
